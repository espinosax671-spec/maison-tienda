...
function createOrderCard(order) {
  ...
  if (order.customer_phone) {
    ...
    actionsHtml += `
      <a href="https://wa.me/57${cleanPhone}?text=${encodeURIComponent(`Hola! Sobre tu pedido #${order.order_number}...`)}" 
         target="_blank" class="btn-whatsapp" style="text-decoration:none; display:inline-flex; align-items:center;">
        💬 WhatsApp
      </a>
    `;
  }
  ...
  if (order.status === "pendiente") {
    actionsHtml += `
      <button class="btn-cancel" data-cancel="${order.id}">❌ Cancelar</button>
      <button class="btn-confirm" data-confirm="${order.id}">✅ Confirmar</button>
    `;
